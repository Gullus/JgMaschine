    ...
    public class AnmeldungAuswertung
    {
          ...
          ArbeitszeitRunden NeuAzRunden, ArbeitszeitBediener NeuAzBediener, CollectionViewSource VsAnzeigeTage)
        {
            ...
            ListeBediener = new JgEntityList<tabBediener>(_Db)
            {
                OnDatenLaden = (d, f) =>
                {
                    var datErsterImMonat = new DateTime(Convert.ToInt32(f.Params["Jahr"]), Convert.ToInt32(f.Params["Monat"]), 1);
                    var datLetzterImMonat = datErsterImMonat.AddMonths(1);

                    return d.tabBedienerSet.Where(w => (w.Status == EnumStatusBediener.Aktiv) && (w.DatumEinstellung < datLetzterImMonat) && ((w.DatumEntlassung == null) || (w.DatumEntlassung >= datErsterImMonat)) && !w.DatenAbgleich.Geloescht).ToList();
                },
                ViewSource = VsBediener
            };
            VsBediener.GroupDescriptions.Add(new PropertyGroupDescription("eStandort.Bezeichnung"));
            VsBediener.SortDescriptions.Add(new System.ComponentModel.SortDescription("Name", System.ComponentModel.ListSortDirection.Ascending));
            ListeBediener.Parameter["Jahr"] = heute.Year;
            ListeBediener.Parameter["Monat"] = heute.Month;
        ...
        public void MonatGeandert()
        {
            ...
            ListeBediener.Parameter["Jahr"] = Jahr;
            ListeBediener.Parameter["Monat"] = Monat;
            ListeBediener.DatenAktualisieren();
